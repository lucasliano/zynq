 ...
 *
 * UTN	 LL	  25/08/24 Added XOR comparison to test
 * </pre>
 *
 * ***************************************************************************
 */
...
/* XOR Constant value defined inside RTL Block (see axis_xor.v) */
#define XOR_CTE		0xDEADBEEF
...
static int CheckData(int Length, u8 StartValue)
{
	...
	for(Index = 0; Index < Length; Index++) {
		...
		sent = sent | (Value << (cnt*8));
		recv = recv | (RxPacket[Index] << (cnt*8));
		cnt++;

		if (cnt == 4)
		{
			xil_printf("D[%d] - Sent: %x - Expected: %x - Received: %x\r\n",
				Index, sent, (sent^XOR_CTE), recv);
			cnt = 0;
			recv = 0;
			sent = 0;
		}
